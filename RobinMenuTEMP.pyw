...
while not done:
    ...
    for x in MAIN_MENU[1:]:
        pygame.draw.rect(screen, BLACK, [0, (BOX[1] * MAIN_MENU.index(x)) + SCRHT, WD -1, HT / 4], 2)
        screen.blit(font.render(x, True, BLACK), [LMARG, (BOX[1] * MAIN_MENU.index(x)) + SCRHT + VCENT])
